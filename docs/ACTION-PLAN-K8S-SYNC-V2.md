...
| A4 | §1 | `AuthorizationPolicy selector: {}` caused ztunnel to omit all HTTP rules (requestPrincipals, paths) and deny ALL east-west pod-to-pod traffic. Fixed by switching to `targetRef: kind: Gateway, name: waypoint` so the policy is scoped to the waypoint (L7) and not evaluated at ztunnel (L4). Security implication: **no JWT enforcement in local k3d** (no waypoint deployed). PeerAuthentication STRICT provides mTLS, which is the correct L4 control. JWT enforcement activates when waypoint is deployed in production. Accepted for local dev. | ✅ resolved (targetRef fix applied) |
