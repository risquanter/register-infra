...
> Same design applies to every mesh-enrolled service with a network probe port — the authoritative list is the `allow-ingress-*-healthcheck` CiliumNetworkPolicies in `infra/k8s/network-policy/`; full mechanism: ADR-INFRA-004 §4 (canonical). PostgreSQL uses exec probes (127.0.0.1). No port-level PERMISSIVE exception remains: SpiceDB's gRPC-probe exception — the last one — was retired 2026-07-15 by switching its kubelet probe to `GET /healthz` on the HTTP gateway (:8080).
